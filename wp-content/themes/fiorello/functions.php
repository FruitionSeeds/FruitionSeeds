...
function bt_rename_coupon_field_on_cart( $translated_text, $text, $text_domain ) {
	...
	if ( 'Coupon:' === $text ) {
		$translated_text = 'Coupon or Gift Certificate:';
	}

	if ('Coupon has been removed.' === $text){
		$translated_text = 'Coupon or Gift Certificate has been removed.';
	}

	if ( 'Apply coupon' === $text ) {
		$translated_text = 'Apply Coupon or Gift Certificate';
	}
...
function bt_rename_coupon_label( $err, $err_code=null, $something=null ){
	$err = str_ireplace("Coupon","Coupon or Gift Certificate ",$err);
	return $err;
}

/* Add variation table */
add_action('woocommerce_single_product_summary', 'add_variation_table', '21');
function add_variation_table() {
	global $product;
	$attributes = $product->get_attributes();
	$output = "[product_table include=" . $product->get_id() . " columns=";
	foreach($attributes as $attr => $value) {
		$output .= "att:" . $attr . ",";
	}
	$output .= "price,stock,buy widths=auto,20,20,20]";
...
/* Adding back in stock notification button to product table */
add_filter( 'wc_product_table_data_add_to_cart', function( $data, $product ) {
	if($product->get_stock_status() == 'outofstock'){
		$data = '<div id="wc_bis_product_form" data-bis-product-id="'.$product->get_id().'" style="margin-left: auto; width: 240px;">';
		if(!is_user_logged_in()){
			$data .= '<input id="wc_bis_email" name="wc_bis_email" type="email" placeholder="Enter Email Address" style="padding: 8px; margin-bottom: 10px; position: relative; top: -1px; width: 141px; font-family: \'Cardo\'; font-size: 14px; width: 100%;"/>';
		}
		$data .= '<button class="button wc_bis_send_form" id="wc_bis_send_form" name="wc_bis_send_form" style="margin-bottom: 0; padding: 10px 15px; width: 100%; font-size: 14px;">Email When Available</button>';
		$data .= '</div>';
	}
    return $data;
}, 10, 2 );

// prepare for move to child theme
require('functions-nlt.php');
